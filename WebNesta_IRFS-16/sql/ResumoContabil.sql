SELECT
'NESTA - RESUMO CONTÁBIL - '+Convert(Char(10), @DATA_SALDO_ANTERIOR,103)+' a '+Convert(varchar(10),@DATA_SALDO_ATUAL ,103)
+' horas@ '+CAST(CONVERT(VARCHAR(50),sysdatetime()) AS CHAR(16))  "Emissão",
...
FO.FONMFORN "Banco", 
OP.OPNMCONT "Descrição",
OP.OPCDAUXI "Nº do Contrato",
PRD.PRPRODES "Produto",
dbo.FUNC_CJTPCTTX(OP.OPIDCONT,'Data Liberação',GETDATE())  "Data Liberação",
...
FROM PHPLANIF T1
WHERE T1.OPIDCONT = OP.OPIDCONT
      AND T1.PHDTEVEN < convert(datetime,@DATA_SALDO_ATUAL,103)) "Contraprestação"
